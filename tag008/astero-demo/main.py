...
class ShipModel(kengi.event.CogObj):
    DASH_DISTANCE = 55
    DELTA_ANGLE = 0.04
    SPEED_CAP = 192
...
class ShipCtrl(kengi.event.EventReceiver):
...
class TinyWorldView(kengi.event.EventReceiver):
    BG_COLOR = (0, 25, 0)
    RAD = 5
...
class IntroV(kengi.event.EventReceiver):
    def __init__(self):
        super().__init__()
...
def init_game():
    ...
    kengi.init('custom', screen_dim=(930, 440))

    my_scr = kengi.core.get_screen()
    SCR_SIZE = my_scr.get_size()
    ...
    p_event = CgmEvent(EngineEvTypes.PAINT, screen=my_scr)


@katasdk.web_animate
def update_game(t_info):
