...
def breakpipe(game, tilemap, x, y):
	...
	for x in range(8):
		entity.Effect(game, self.hitbox.center, 'effect_explosion_003', game.scene.behind, angle=randint(0,360), speed=4)
		entity.Effect(game, self.hitbox.center, 'effect_explosion_004', game.scene.behind, angle=randint(0,360), speed=4)
	adj = (
			(x-1,y),
			(x+1,y),
			(x,y-1),
