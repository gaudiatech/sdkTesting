...
ASSET_ALIASES = {
    'musiquefond': 'assets/chiptronic.ogg',
    'bruitage_menu': 'assets/coinlow.wav',
    'valid_menu': 'assets/coinhigh.wav',
    'explo': 'assets/explo_basique.wav',
    'quake': 'assets/crumble.wav',
    'fond_gameover': 'assets/img_bt_rouge.png'
}

# dummy classes, temp patch{{
class EventReceiver:
    pass
...
SCRWid = 440
SCRHei = 250
surf = pygame.display.set_mode((SCRWid, SCRHei))
...
last_t = None
MVT_SPEED = 1.73  # the higher the value, the more the avatar will move
color = 'red'
asset0 = None
...
def game_init():
    global surf, asset0
    asset0 = pygame.transform.scale(pygame.image.load(
        ASSET_ALIASES['fond_gameover']
    ), (128, 88))


@katasdk.web_animate
def game_update(tinfo):
    global gameover, surf, phase, last_t, color
    ...
    phase = (phase + MVT_SPEED*dt)
    if phase > 2*math.pi:
        phase = phase - 2*math.pi
    position[0] = BASEPTX + 139*math.sin(phase)
    position[1] = BASEPTY + 88*math.cos(math.pi/2+3*phase)

    # - paint
    surf.fill('orange')
    pygame.draw.rect(surf, 'brown', (89, 115, 33, 45), 6)
    surf.blit(asset0, (0, 0))

    pygame.draw.circle(surf, color, position, 22)
    pygame.display.flip()
    clock.tick(30)
